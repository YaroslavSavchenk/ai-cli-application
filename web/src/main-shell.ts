/**
 * The app shell (split from `main.ts`, O8, 2026-09-23): `buildShell` — the
 * top bar, the drawers, the Files panel, the pane grid, the tab strip and the
 * statusline, wired together — plus the global keyboard chords, the session
 * and runtime polls, and the reconnect / restart takeovers they raise.
 * `main.ts` keeps the entry: the stylesheets, the boot panel and `boot()`,
 * which calls `buildShell` once the server state is in. Sibling: `main.ts`.
 */
...
import { initFileDrop, type DropRequest } from './ui/filedrop.ts';
...
export function buildShell(root: HTMLDivElement, prefs: UiPrefs | undefined): void {
  // ---- top bar (Nocturne A2) -----------------------------------------------
