...
Status: see the table below (updated 2026-09-26; shipped as v0.4.0 on 2026-09-23). Conventions: `.claude/plans/README.md`.
...
| C2 | Mascot waits while background subagents / workflows run, comes for Claude's questions; the pane readout follows the same rule | landed | `.claude/plans/nocturne/PLAN-C2.md` | 2026-09-26 |
...
- DONE 2026-09-14 (A8 landed): 0 alias references remain; the `LEGACY ALIAS LAYER` block and marker are deleted (tokens.css = 144 tokens in 8 sections), guarded by `tests/ui/ui-a8-tokens.test.ts`. The paragraph below is the pre-A8 warning, kept as history.
...
### B11. Session state: Working vs. Waiting for you (NOTED 2026-09-22, user's ask on the B7 check; LANDED 2026-09-22, spec `.claude/plans/nocturne/PLAN-B11.md`)
