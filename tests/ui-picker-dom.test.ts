 ...
 * through the REAL module on the DOM double in `tests/fake-dom.ts`, opened the
 ...
 * (`tests/scaffold.test.ts`, `tests/ui-api-auth.test.ts`) and of the pure path
 * helpers (`tests/ui-newproject.test.ts`). Nothing clicked the picker. These
...
test("the Home quick zone resolves through fsList: an unknown home asks the backend for its OWN $HOME", async () => {
  // `/home/...` is nowhere in picker.ts (pinned in tests/ui-picker-a7.test.ts):
