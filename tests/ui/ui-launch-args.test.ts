 ...
 * the plain-language copy layered over it: `composeArgs`,
 * `parseCustomCommand`, the vocabularies, the session kinds and shells
 * (`composeSpawn`, `shellSpawn`) and the project-default resolvers. The
 * Nocturne A4 card/label tables are in `ui-launch-args-a4.test.ts`, the B5
 * agents (Codex, Gemini, Grok) in `ui-launch-args-b5.test.ts`.
 *
