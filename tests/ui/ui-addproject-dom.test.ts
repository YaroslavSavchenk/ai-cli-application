 ...
 * driven through the REAL module on the DOM double in `tests/helpers/fake-dom.ts`.
 *
 * WHY THIS FILE EXISTS. A7 restyled this dialog, and every test it had was a
 * SOURCE scan (`tests/ui/ui-addproject-a7.test.ts` for the style contract,
 * `tests/ui/ui-newproject-add.test.ts` for the create-vs-add wiring) or a pure
 * model test (`tests/ui/ui-newproject*.test.ts`, `tests/ui/ui-github-model.test.ts`).
