...
/**
 * Tokens with no reader that STAY. `tests/nocturne-tokens.test.ts` (a) requires
...
test('(a) the alias marker text survives nowhere in the frontend', () => {
  ...
  const stale = readdirSync(dir)
    .filter((n) => n.endsWith('.ts') && !allowed.has(n))
  ...
  assert.ok(
    readFileSync(join(dir, 'nocturne-tokens.test.ts'), 'utf8').includes(MARKER),
