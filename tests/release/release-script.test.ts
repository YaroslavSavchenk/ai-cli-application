 ...
 * tests/server/github-clone-success.test.ts). Every double appends its own argv to
...
test('workflows: CI and release both call verify.yml, and publishing needs it', async () => {
  ...
  // build jobs; tests/release/release-workflow.test.ts owns the rest of the graph.)
  assert.match(
    release,
