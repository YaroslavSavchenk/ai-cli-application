 ...
 * (`tests/editor-fixture.ts`, `tests/fs-fixture.ts`).
 *
...
import { BINARY_PATH, GONE_PATH, GONE_TEXT as DIFF_GONE_TEXT, HEAD, diffOf } from './commits-fixture.ts';
import {
  CHANGED_TEXT,
  GONE_TEXT,
  NOT_TEXT_TEXT,
...
test('the mock is GONE: no placeholder branch, no invented file content, no api import', () => {
  ...
  for (const dir of [join(projectRoot, 'web', 'src'), join(projectRoot, 'web', 'src', 'ui'), join(projectRoot, 'tests')]) {
