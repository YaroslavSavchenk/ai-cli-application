 ...
 * (`tests/ui-settings-a7.test.ts` for the `sg-` block,
 * `tests/ui-addproject-a7.test.ts` for the `ap-`/`gh-` block). Those two scope
...
test('the four Legacy rules the folder-picker restyle deleted have zero users left, anywhere', () => {
  ...
  // read. `tests/ui-picker-a7.test.ts` checks them against picker.ts and ui/*.ts;
