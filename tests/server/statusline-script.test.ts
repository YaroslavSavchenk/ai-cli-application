 ...
 * <prefs.json>`, pipe a payload on stdin, read one line of stdout (runners and
 ...
 *   4. The git branch is cached per `session_id`, in the data dir, ~5 s — and
 *      that cache file is untrusted input.
 *
 ...
 *
 * NOT claimed here: the optional fourth argument, the pane-bar SNAPSHOT
 * (Nocturne B1) — `tests/server/statusline-script-snapshot.test.ts`.
 */
