 ...
 * seam tests/server/fs-entries.test.ts uses), so the boundary, the data-dir refusal and
...
test('/api/fs/mkdir — the PICKER’s new-folder button — still has no boundary at all, on purpose', async () => {
  // The other half of the asymmetry pinned in tests/server/fs-entries.test.ts: the
